...
function tableEmbed(session, { phase = 'play', result = null, payout = 0, note = null } = {}) {
    ...
    const lines = [
        ...
        '   ╚══════════════════════════════╝',
        '```',
        `❄️ Aposta **${fmt(session.amount)}**${session.doubled ? ' · *doubled*' : ''}`,
        '',
        `🏠 **Dealer**`,
    ...
    if (result) {
        ...
        if (payout > 0) lines.push(`✨ +❄️ **${fmt(payout)}**`);
        if (result === 'lose' || result === 'bust')
            lines.push(`💫 Prejuízo −❄️ **${fmt(session.amount)}**`);
        if (result === 'push') lines.push('Aposta devolvida na íntegra.');
        lines.push(`💼 Saldo: ❄️ **${fmt(flocos.get(session.userId))}**`);
    } else {
...
function settle(session) {
    ...
    if (d > 21 || p > d) {
        const pay = bet * 2;
        flocos.add(session.userId, pay, { reason: 'bj win' });
        return { result: 'win', payout: pay };
    }
    if (p === d) {
        flocos.add(session.userId, bet, { reason: 'bj push' });
...
module.exports = {
    ...
    async execute(message, args) {
        ...
        flocos.remove(message.author.id, bet.amount, { reason: 'bj bet' });
        const session = startHand(message.author.id, bet.amount);

        if (isBlackjack(session.player)) {
            if (isBlackjack(session.dealer)) {
                flocos.add(session.userId, session.amount, { reason: 'bj push' });
            ...
            flocos.add(session.userId, pay, { reason: 'bj natural' });
            sessions.delete(session.userId);
            return message.reply({
    ...
    async handleComponent(interaction) {
        ...
        if (action === 'again') {
            ...
            const bet = resolveBet(String(amount), flocos.get(userId), { label: '❄️' });
            if (!bet.ok) return interaction.reply({ content: `❌ ${bet.error}`, ephemeral: true });
            if (sessions.has(userId)) sessions.delete(userId);
            flocos.remove(userId, bet.amount, { reason: 'bj again' });
            const session = startHand(userId, bet.amount);
            if (isBlackjack(session.player)) {
                if (isBlackjack(session.dealer)) {
                    flocos.add(userId, session.amount, { reason: 'bj push' });
                ...
                const pay = Math.floor(session.amount * 2.5);
                flocos.add(userId, pay, { reason: 'bj natural' });
                sessions.delete(userId);
                return interaction.update({
                    embeds: [tableEmbed(session, { phase: 'end', result: 'bj', payout: pay })],
                    components: [againRow(userId, session.amount)]
        ...
        if (!session)
            return interaction.reply({ content: '⏱️ Mão expirada.', ephemeral: true });
        ...
        if (action === 'double') {
            ...
            if (flocos.get(userId) < session.amount)
                return interaction.reply({
                    content: `❌ ❄️ insuficiente. Precisa de **${fmt(session.amount)}**.`,
                    ephemeral: true
                });
            flocos.remove(userId, session.amount, { reason: 'bj double' });
            session.amount *= 2;
