...
function profileEmbed(user, profile) {
    ...
    try {
        ...
        const parts = [];
        if (eq.weapon) parts.push(`⚔️ ${eq.weapon.name || eq.weapon.id}`);
        if (eq.armor) parts.push(`🛡️ ${eq.armor.name || eq.armor.id}`);
        if (eq.accessory) parts.push(`💍 ${eq.accessory.name || eq.accessory.id}`);
    ...
    const emb = new EmbedBuilder()
        ...
        .setDescription(
            [
                ...
                `\`${bar}\``,
                '',
                `❤️ **HP** \`${Number(hpMax).toLocaleString('pt-BR')}\` · 🔷 **Mana** \`${Number(manaMax).toLocaleString('pt-BR')}\``,
...
async function startPhotoWait(user, draft) {
    try {
        ...
        const cls = resolveClass({ classId: draft.classId });
        await dm.send({
            embeds: [
                new EmbedBuilder()
                    .setColor(cls.color || 0xa78bfa)
    ...
    async handleComponent(interaction) {
        ...
        if (id.startsWith('j:attrplus:')) {
            ...
            try {
                ...
                await interaction
                    .reply({
                        content: '✅ **' + meta.label + '** aumentou! Abra de novo com `O.j atributos`.',
                        flags: eph
                    })
                    .catch(() => {});
            }
