...
function createMatch({ mode = '1v1', teamA = [], teamB = [], bet = 0 }) {
    ...
    const match = {
        id, mode: aIds.length === 1 ? '1v1' : 'team', fighters, teamA: aIds, teamB: bIds,
        turnOrder, turnIndex: 0, currentId: turnOrder[0], turnEndsAt: Date.now() + TURN_MS,
        log: [{ t: Date.now(), text: '⚔️ A arena se abre. Que o Éter decida o vencedor.' }],
        status: 'active', winnerTeam: null, bet: Number(bet) || 0, lastEffect: null, createdAt: Date.now()
    };
    arenas.set(id, match);
...
function applyRewards(match, winners, losers) {
    const avgLoserLevel = losers.reduce((s, id) => s + (match.fighters[id]?.level || 0), 0) / Math.max(1, losers.length);
    for (const id of winners) {
        ...
        xp.addXp(id, xpGain);
        match.log.push({ t: Date.now(), text: `🎁 ${match.fighters[id].name} recebe baú **${chest.name}** e +${xpGain} XP.` });
        if (chest.item) try { player.addItem(id, chest.item); } catch (_) {}
    }
    for (const id of losers) {
        try {
            ...
            cur.level = level; cur.xp = totalXp;
            for (const k of Object.keys(cur.attrs || {})) cur.attrs[k] = Math.max(0, Math.floor(Number(cur.attrs[k] || 0) / 2));
            data[id] = cur; store.save('xp.json', data);
            match.log.push({ t: Date.now(), text: `💀 ${match.fighters[id].name} perdeu metade do nível e dos atributos.` });
        } catch (e) {
...
function publicState(match, asUserId) {
    return {
        id: match.id, mode: match.mode, status: match.status, winnerTeam: match.winnerTeam,
        currentId: match.currentId, turnEndsAt: match.turnEndsAt, turnMs: TURN_MS,
        teamA: match.teamA.map(id => publicFighter(match.fighters[id])),
        teamB: match.teamB.map(id => publicFighter(match.fighters[id])),
        log: match.log.slice(-40), lastEffect: match.lastEffect, you: asUserId || null,
        yourTurn: match.currentId === asUserId
    };
}

module.exports = { TURN_MS, createMatch, getMatch, applyMove, publicState, loadFighter, rollChest };
