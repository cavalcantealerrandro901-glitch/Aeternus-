...
function publicState(fight, viewerId) {
    ...
    return {
        ...
        mobility: [
            { key: 'step_left', label: 'Esquerda', emoji: '⬅️', cost: 0, disabled: !!fight.over },
            { key: 'step_right', label: 'Direita', emoji: '➡️', cost: 0, disabled: !!fight.over }
        ],
        lastEffect: fight.lastEffect || null,
        arena: { type: 'circle', radiusM: 2 }
    };
}
...
function createArena({ a, b, bet = 0, channelId = null, guildId = null }) {
    ...
    const fight = {
        id,
        a: { ...a, lane: -1, lastAnim: null },
        b: { ...b, lane: 1, lastAnim: null },
        turn: first,
        round: 1,
        bet: Number(bet) || 0,
        over: false,
...
function applyMove(fightId, playerId, kind) {
    ...
    const allowed = ['attack', 'heavy', 'defend', 'special', 'step_left', 'step_right'];
    ...
    const costs = costsFor(actor.classId);
    const moves = movesFor(actor.classId);
    const isA = actor === fight.a;

    if (kind === 'step_left' || kind === 'step_right') {
        if (typeof actor.lane !== 'number') actor.lane = isA ? -1 : 1;
        const dir = kind === 'step_left' ? -1 : 1;
        const next = Math.max(-1, Math.min(1, actor.lane + (isA ? dir : -dir)));
        if (next === actor.lane) {
            return { ok: false, error: 'Não há espaço para esse lado na arena.' };
        }
        actor.lane = next;
        actor.lastAnim = kind;
        actor.defending = false;
        const sideLabel = kind === 'step_left' ? 'esquerda' : 'direita';
        fight.lastLog = `👟 **${actor.name}** se desloca para a ${sideLabel} da arena.`;
        fight.log.push(fight.lastLog);
        fight.lastEffect = { kind, actorId: actor.id, lane: actor.lane };
        actor.mana = Math.min(actor.maxMana, actor.mana + 2);
        fight.turn = fight.turn === fight.a.id ? fight.b.id : fight.a.id;
        fight.round += 1;
        if (!fight.over) {
            const nextF = fight.turn === fight.a.id ? fight.a : fight.b;
            if (nextF.isBot) setTimeout(() => botMove(fight.id), 700);
        }
    ...
    if (kind === 'defend') {
        ...
        actor.specialCd = 3;
        actor.defending = false;
        actor.lastAnim = 'special';
        if (typeof actor.lane === 'number') {
            actor.lane = actor === fight.a ? Math.min(1, actor.lane + 1) : Math.max(-1, actor.lane - 1);
        }
        ...
        fight.lastLog =
            ...
            (heal ? ` · +${heal} HP` : '');
        fight.log.push(fight.lastLog);
        fight.lastEffect = { kind: 'special', actorId: actor.id, targetId: target.id, lane: actor.lane };
    } else {
        ...
        fight.lastLog =
            ...
            (heal ? ` · +${heal} HP` : '');
        fight.log.push(fight.lastLog);
        fight.lastEffect = { kind, actorId: actor.id, targetId: target.id, lane: actor.lane };
    }
