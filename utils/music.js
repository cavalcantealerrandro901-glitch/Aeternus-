...
function isYoutubeBlocked(errMsg) {
...
/**
 * Prioridade: YouTube → se bloquear/falhar → SoundCloud (e outras).
 */
async function loadTracks(query) {
    ...
    const attempts = [];
    const forced = /^(ytsearch|scsearch|spsearch|ytmsearch|amsearch|dzsearch):/i.test(q);

    if (forced) {
        attempts.push({ id: q, source: q.split(':')[0] });
    } else if (/^https?:\/\//i.test(q)) {
        // Link direto: tenta o próprio link primeiro
        attempts.push({ id: q, source: 'url' });
        if (isYoutubeUrl(q)) {
            // Se o YouTube bloquear o link, tenta busca pelo ID / query residual no SC
            const idMatch = q.match(/(?:v=|youtu\.be\/|shorts\/)([a-zA-Z0-9_-]{6,})/);
            if (idMatch) {
                attempts.push({ id: `ytsearch:${idMatch[1]}`, source: 'ytsearch' });
            }
            // fallback genérico: SoundCloud com o trecho útil da URL
            const hint = q.replace(/^https?:\/\//i, '').slice(0, 80);
            attempts.push({ id: `scsearch:${hint}`, source: 'scsearch' });
        } else if (!isSoundcloudUrl(q)) {
            attempts.push({ id: `ytsearch:${q}`, source: 'ytsearch' });
            attempts.push({ id: `scsearch:${q}`, source: 'scsearch' });
        }
    } else {
        // Texto livre: YouTube primeiro, depois SoundCloud
        attempts.push({ id: `ytsearch:${q}`, source: 'ytsearch' });
        attempts.push({ id: `ytmsearch:${q}`, source: 'ytmsearch' });
        attempts.push({ id: `scsearch:${q}`, source: 'scsearch' });
        attempts.push({ id: `ytsearch:${q} audio`, source: 'ytsearch' });
    }
    ...
    for (const { id, source } of attempts) {
        // Se YouTube já falhou por bloqueio e a tentativa ainda é YT, pula para ir mais rápido ao fallback
        if (youtubeFailed && (source === 'ytsearch' || source === 'ytmsearch' || (source === 'url' && isYoutubeUrl(id)))) {
            continue;
        }
        try {
            console.log(`[lavalink] loadtracks (${source}): ${id.slice(0, 90)}`);
            const data = await loadTracksOnce(id);
            const { tracks, error } = extractTracks(data);
            if (tracks.length) {
                if (youtubeFailed && source === 'scsearch') {
                    console.log('[lavalink] fallback SoundCloud OK após bloqueio/falha do YouTube');
                }
                return { data, tracks, source };
            }
            if (error) {
                errors.push(`${source}: ${error}`);
                if (source === 'ytsearch' || source === 'ytmsearch' || (source === 'url' && isYoutubeUrl(q))) {
                    youtubeFailed = true;
                    console.warn(`[lavalink] YouTube falhou (${error.slice(0, 80)}) — tentando outra fonte…`);
                }
            }
        } catch (e) {
            ...
            if (source === 'ytsearch' || source === 'ytmsearch' || (source === 'url' && isYoutubeUrl(q))) {
                youtubeFailed = true;
                console.warn(`[lavalink] YouTube erro (${msg.slice(0, 80)}) — tentando outra fonte…`);
            }
        }
    }
    ...
    throw new Error(
        `Não achei a música.\n${detail}\n\n` +
            `Prioridade: **YouTube** → se bloquear → **SoundCloud**.\n` +
            `Dicas:\n` +
            `• Tente um **link** do YouTube ou SoundCloud\n` +
            `• Use \`O.tocar scsearch:nome\` para forçar SoundCloud\n` +
            `• Use \`O.tocar ytsearch:nome\` para forçar YouTube`
    );
}
