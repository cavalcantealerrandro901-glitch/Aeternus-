...
const DEFAULT = {
    ...
    sticky: { enabled: false, channelId: null, content: '', every: 8 },
    autoPublish: { enabled: false, channelIds: [] },
    memberCounter: { enabled: false, channelId: null, format: '👥 Membros: {count}' },
    autoReact: { enabled: false, channelId: null, emojis: ['👍', '❤️'] },
    autoThread: { enabled: false, channelId: null, nameFormat: 'Discussão · {user}' },
    dmWelcome: { enabled: false, message: 'Olá {user}! Bem-vindo ao **{server}**.' },
    mentionGuard: { enabled: false, maxMentions: 5, punish: 'delete' },
    voiceHub: { enabled: false, channelId: null, createTemp: false },
    partnership: {
        enabled: true,
        channelId: null,
        roleId: null,
...
function setSettings(guildId, patch) {
    ...
    if (patch.drops?.requirements) {
        all[guildId].drops = all[guildId].drops || {};
        all[guildId].drops.requirements = {
            ...(all[guildId].drops.requirements || {}),
            ...patch.drops.requirements
        };
        if (Array.isArray(patch.drops.requirements.requiredRoleIds)) {
            all[guildId].drops.requirements.requiredRoleIds =
                patch.drops.requirements.requiredRoleIds;
        }
        if (Array.isArray(patch.drops.requirements.blockedRoleIds)) {
            all[guildId].drops.requirements.blockedRoleIds =
                patch.drops.requirements.blockedRoleIds;
        }
        if (Array.isArray(patch.drops.requirements.bypassRoleIds)) {
            all[guildId].drops.requirements.bypassRoleIds =
                patch.drops.requirements.bypassRoleIds.map(String);
        }
    }
    if (patch.shop?.vips) {
