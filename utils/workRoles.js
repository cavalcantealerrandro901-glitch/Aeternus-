/**
 * Cargos de trabalho no Discord: cria, atribui e troca ao subir de nível.
 * Requer permissão Gerenciar Cargos e o cargo do bot acima destes.
 */
...
async function ensureRoles(guild) {
    if (!guild?.id) return {};
    if (!canManage(guild)) {
        console.warn('[workRoles] sem permissão Gerenciar Cargos em', guild.id);
        return loadMap(guild.id);
    }

    const map = { ...loadMap(guild.id) };
    let changed = false;
        ...
        if (!role) {
            try {
                role = await guild.roles.create({
                    name,
                    colors: COLORS[rank.id] ?? 0x99aab5,
                    hoist: false,
                    mentionable: false,
                    reason: 'Cargo automático do sistema de trabalho (Aeternus)'
                });
...
async function syncMember(member, rankId) {
    if (!member?.guild || rankId == null) return { ok: false, reason: 'invalid' };
    if (!canManage(member.guild)) return { ok: false, reason: 'no_perm' };

    const map = await ensureRoles(member.guild);
    const targetId = map[String(rankId)] ?? map[rankId];
    ...
    try {
        const toRemove = manageable.filter(
            (id) => id !== targetId && member.roles.cache.has(id)
        );
        if (toRemove.length) {
            await member.roles.remove(toRemove, 'Troca de cargo de trabalho');
        }
