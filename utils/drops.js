...
function getRequirements(guildIdOrDrop, maybeDrop) {
    ...
    const bypassRoleIds = over.bypassRoleIds || base.bypassRoleIds || [];
    return {
        ...
        bypassRoleIds: Array.isArray(bypassRoleIds) ? bypassRoleIds.map(String) : [],
...
function checkRequirements(member, drop) {
    ...
    for (const rid of req.blockedRoleIds || []) {
        if (member.roles.cache.has(String(rid))) {
            fails.push('cargo bloqueado neste drop');
            return { ok: false, fails, bypass: false };
        }
    }

    const bypassIds = (req.bypassRoleIds || []).map(String);
    if (bypassIds.length && bypassIds.some((rid) => member.roles.cache.has(rid))) {
        return { ok: true, fails: [], bypass: true };
    }
    ...
    return { ok: !fails.length, fails, bypass: false };
}
...
function formatDropPanelInfo(guild, guildId) {
    ...
    const bypass = [];
    for (const rid of req.bypassRoleIds || []) {
        const role = guild?.roles?.cache?.get(String(rid));
        bypass.push(role ? `${role}` : String(rid));
    }
    return { vipLines, blocked, bypass };
}
