...
function parsePrize(text) {
    ...
    const m = t.match(/^(\d+(?:[.,]\d+)?[km]?)\s*(éter|eter|flocos?|cristais?|✨|❄️|💠)?$/i);
    if (m) {
        ...
        if (amount > 0) {
            return {
                type: 'eter',
                amount,
                label: `✨ ${amount.toLocaleString('pt-BR')} éter`
            };
        }
    }
...
function parseReqFlags(text, message) {
    ...
    else out.minLevel = num(/(?:nivel|nível|level|xp)[:\s]+(\d+)/);
    out.minInvites = num(/(?:invites?|convites?)[:\s]+(\d+)/);
    out.minFlocos = num(/(?:flocos?|éter|eter)[:\s]+(\d+)/);
...
function payPrize(userId, prize) {
    ...
    eter.add(userId, prize.amount, { reason: 'drop' });
    return true;
}
...
function checkRequirements(member, drop) {
    ...
    if (req.minFlocos > 0 && eter.get(member.id) < req.minFlocos)
        fails.push(`éter: ${eter.get(member.id)}/${req.minFlocos}`);
    if (req.minCristais > 0 && eter.get(member.id) < req.minCristais)
        fails.push(`éter: ${eter.get(member.id)}/${req.minCristais}`);
