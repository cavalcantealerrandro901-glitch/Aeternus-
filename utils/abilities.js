...
const ACTIVE_SLOTS = 4, PASSIVE_SLOTS = 5;
function getAbility(id) { return ABILITIES[id] || null; }
function listByKind(kind) { return Object.values(ABILITIES).filter(a => a.kind === kind); }
...
function sumPassiveMods(userId) {
  ...
  for (const p of passive) {
    if (!p?.mods) continue;
    for (const [k, v] of Object.entries(p.mods)) mods[k] = (mods[k] || 0) + Number(v);
  }
  return mods;
}
module.exports = { ABILITIES, ACTIVE_SLOTS, PASSIVE_SLOTS, getAbility, listByKind, loadLoadout, saveLoadout, equipAbility, unequipSlot, getEquippedAbilities, sumPassiveMods };
