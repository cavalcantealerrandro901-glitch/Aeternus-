module.exports = (guild, user, userAvatarUrl, config, channels, categories = [], roles = []) => {
    ...
    const tBtn = (tickets.buttonLabel || 'Abrir Ticket').replace(/"/g, '&quot;');
...
<p class="desc">Painel com botão para os membros abrirem tickets privados.</p>
...
<label>Canal do painel (onde fica o botão)</label>
...
<input id="ticket-btn" value="${tBtn}">
...
function getTickets(){return{enabled:document.getElementById('toggle-tickets').checked,panelChannel:document.getElementById('ticket-panel').value||null,category:document.getElementById('ticket-category').value||null,supportRole:document.getElementById('ticket-role').value||null,embedTitle:document.getElementById('ticket-title').value||'',embedDescription:document.getElementById('ticket-desc').value||'',buttonLabel:document.getElementById('ticket-btn').value||'Abrir Ticket',embedColor:document.getElementById('ticket-color').value||'#7c3aed',openMessage:document.getElementById('ticket-open-msg').value||''}};
