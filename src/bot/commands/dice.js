...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(o => o.setName('numero').setDescription('Número de 1 a 6').setRequired(true).setMinValue(1).setMaxValue(6)),
    aliases: ['dado'],
    ...
    async run(userId, guildId, amount, pick) {
        ...
        return { roll, pick, win, prize, amount, total: user.almas };
    },
    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setTitle('🎲 Dice')
            .setDescription(`Você escolheu **${r.pick}** · Saiu **${r.roll}**\n` +
                (r.win
                    ? `Acertou! Ganhou ${formatAlmas(r.prize, interaction.guild.id)}`
                    : `Errou. Perdeu ${formatAlmas(r.amount, interaction.guild.id)}`) +
    ...
    async executePrefix(message, args) {
        ...
        if (!amount || !pick || pick < 1 || pick > 6) return message.reply('Uso: `dice <quantidade> <1-6>`');
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Você escolheu **${r.pick}** · Saiu **${r.roll}**\n` +
                (r.win
                    ? `Acertou! Ganhou ${formatAlmas(r.prize, message.guild.id)}`
                    : `Errou. Perdeu ${formatAlmas(r.amount, message.guild.id)}`) +
